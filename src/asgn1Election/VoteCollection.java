...
public class VoteCollection implements Collection {
	 ...
	 */
	// Constructor that throws an exception if the number of candidates is within the set range of candidates.
	...
	public void countPrefVotes(TreeMap<CandidateIndex, Candidate> cds,
			CandidateIndex elim) {
		// Loops through the list of votes, counts the votes
		for(int vote = 0; vote < voteList.size();vote++){
			cds.get(getPrimaryKey(voteList.get(vote))).incrementVoteCount();
		}
	}
	...
	public void emptyTheCollection() {
		// Clears the variables for the collection.
		voteList.clear();
		formalCount = 0;
		informalCount = 0;
	}
	...
	public void includeFormalVote(Vote v) {
		// Adds the vote and increments the number of formal votes.
		this.voteList.add(v);
		this.formalCount = formalCount + 1;
	}
	...
	public void updateInformalCount() {
		// Increments then number of informal votes.
		this.informalCount = informalCount + 1;
	}
